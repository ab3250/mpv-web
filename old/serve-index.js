 ...
 */
//  added
const audioFilesRegExp = require('./variables.js').audioFilesRegExp
...
serveIndex.html = function _html (req, res, files, next, dir, showUp, icons, path, view, template, stylesheet) {
  ...
  stat(path, files, function (err, stats) {
    ...
    // ***********************************************************added to insert play all and remove non-audio
    fileList.forEach(function (value, index, collection) { // remove non-audio files from file list
      var isDir = value.stat && value.stat.isDirectory() // keep dir and music files
      if (!audioFilesRegExp.test(value.name) && !isDir) {
...
function createHtmlFileList (files, dir, useIcons, view) {
  var html = '<ul id="files2" class="view-' + escapeHtml(view) + '">' +
    (view === 'details' ? (
      '<li class="header">' +
      '<span>Name</span>' +
      '</li>') : '')
...
function htmlPath (dir) {
  var parts = dir.substring(6, dir.length).split('/') // added .substring(6, dir.length) *********************
